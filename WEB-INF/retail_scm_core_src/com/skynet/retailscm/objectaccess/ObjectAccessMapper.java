...
public class ObjectAccessMapper extends BaseRowMapper<ObjectAccess>{
	...
	protected ObjectAccess internalMapRow(ResultSet rs, int rowNumber) throws SQLException{
		ObjectAccess objectAccess = getObjectAccess();		
 		...
 		setDisplayName(objectAccess, rs, rowNumber); 		
	...
	protected void setDisplayName(ObjectAccess objectAccess, ResultSet rs, int rowNumber) throws SQLException{
	
		//there will be issue when the type is double/int/long
		String displayName = rs.getString(ObjectAccessTable.COLUMN_DISPLAY_NAME);
		if(displayName == null){
			//do nothing when nothing found in database
			return;
		}
