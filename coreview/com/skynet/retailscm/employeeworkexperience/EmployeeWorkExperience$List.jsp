...
<c:set var="employeeWorkExperienceList" value="${result.employeeWorkExperienceList}"/>
...
<c:if test="${ empty employeeWorkExperienceList}" >
 ...
 <c:if test="${not empty result.employeeWorkExperienceList}" >
    
    
<%

 	SmartList list=(SmartList)pageContext.getAttribute("employeeWorkExperienceList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/employeeWorkExperienceList;employeeWorkExperienceListCurrentPage=${page.pageNumber}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${result.employeeWorkExperienceList}">
				<tr currentVersion='${item.version}' id="employeeWorkExperience-${item.id}" ><td><a class="link-action-removed" href="./employeeWorkExperienceManager/loadEmployeeWorkExperienceDetail/${item.id}/"> ${item.id}</a></td>
...
</c:if><c:if test="${param.referName ne 'start'}">	<td contenteditable='true' class='edit-value'  propertyToChange='start' storedCellValue='${item.start}' prefix='${ownerBeanName}Manager/updateEmployeeWorkExperience/${result.id}/${item.id}/'><fmt:formatDate pattern="yyyy-MM-dd" value="${item.start}" /></td>
