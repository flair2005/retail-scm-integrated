...
<c:set var="storageSpaceList" value="${result.storageSpaceList}"/>
...
<c:if test="${ empty storageSpaceList}" >
 ...
 <c:if test="${not empty result.storageSpaceList}" >
    
    
<%

 	SmartList list=(SmartList)pageContext.getAttribute("storageSpaceList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/storageSpaceList;storageSpaceListCurrentPage=${page.pageNumber}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${result.storageSpaceList}">
				<tr currentVersion='${item.version}' id="storageSpace-${item.id}" ><td><a class="link-action-removed" href="./storageSpaceManager/loadStorageSpaceDetail/${item.id}/"> ${item.id}</a></td>
...
</c:if><c:if test="${param.referName ne 'latitude'}">	<td contenteditable='true' class='edit-value'  propertyToChange='latitude' storedCellValue='${item.latitude}' prefix='${ownerBeanName}Manager/updateStorageSpace/${result.id}/${item.id}/'>${item.latitude}</td>
