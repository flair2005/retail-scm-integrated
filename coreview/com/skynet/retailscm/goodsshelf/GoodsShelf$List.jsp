...
<c:set var="goodsShelfList" value="${result.goodsShelfList}"/>
...
<c:if test="${ empty goodsShelfList}" >
 ...
 <c:if test="${not empty result.goodsShelfList}" >
    
    
<%

 	SmartList list=(SmartList)pageContext.getAttribute("goodsShelfList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/goodsShelfList;goodsShelfListCurrentPage=${page.pageNumber}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${result.goodsShelfList}">
				<tr currentVersion='${item.version}' id="goodsShelf-${item.id}" ><td><a class="link-action-removed" href="./goodsShelfManager/loadGoodsShelfDetail/${item.id}/"> ${item.id}</a></td>
...
</c:if><c:if test="${param.referName ne 'storageSpace'}">	<td ><a href='./storageSpaceManager/loadStorageSpaceDetail/${item.storageSpace.id}/'>${item.storageSpace.id}</a></td>
</c:if><c:if test="${param.referName ne 'supplierSpace'}">	<td ><a href='./supplierSpaceManager/loadSupplierSpaceDetail/${item.supplierSpace.id}/'>${item.supplierSpace.id}</a></td>
</c:if><c:if test="${param.referName ne 'damageSpace'}">	<td ><a href='./damageSpaceManager/loadDamageSpaceDetail/${item.damageSpace.id}/'>${item.damageSpace.id}</a></td>
</c:if>
				<td>
