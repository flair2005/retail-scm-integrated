...
<c:set var="accountingPeriodList" value="${result.accountingPeriodList}"/>
...
<c:if test="${ empty accountingPeriodList}" >
 ...
 <c:if test="${not empty result.accountingPeriodList}" >
    
    
<%

 	SmartList list=(SmartList)pageContext.getAttribute("accountingPeriodList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/accountingPeriodList;accountingPeriodListCurrentPage=${page.pageNumber}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${result.accountingPeriodList}">
				<tr currentVersion='${item.version}' id="accountingPeriod-${item.id}" ><td><a class="link-action-removed" href="./accountingPeriodManager/loadAccountingPeriodDetail/${item.id}/"> ${item.id}</a></td>
