...
<c:set var="employeeAttendanceList" value="${result.employeeAttendanceList}"/>
...
<c:if test="${ empty employeeAttendanceList}" >
 ...
 <c:if test="${not empty result.employeeAttendanceList}" >
    
    
<%

 	SmartList list=(SmartList)pageContext.getAttribute("employeeAttendanceList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/employeeAttendanceList;employeeAttendanceListCurrentPage=${page.pageNumber}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${result.employeeAttendanceList}">
				<tr currentVersion='${item.version}' id="employeeAttendance-${item.id}" ><td><a class="link-action-removed" href="./employeeAttendanceManager/loadEmployeeAttendanceDetail/${item.id}/"> ${item.id}</a></td>
...
</c:if><c:if test="${param.referName ne 'enterTime'}">	<td contenteditable='true' class='edit-value'  propertyToChange='enterTime' storedCellValue='${item.enterTime}' prefix='${ownerBeanName}Manager/updateEmployeeAttendance/${result.id}/${item.id}/'><fmt:formatDate pattern="yyyy-MM-dd" value="${item.enterTime}" /></td>
