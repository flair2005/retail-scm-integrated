...
<c:set var="warehouseList" value="${result.warehouseList}"/>
...
<c:if test="${ empty warehouseList}" >
 ...
 <c:if test="${not empty result.warehouseList}" >
    
    
<%

 	SmartList list=(SmartList)pageContext.getAttribute("warehouseList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/warehouseList;warehouseListCurrentPage=${page.pageNumber}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${result.warehouseList}">
				<tr currentVersion='${item.version}' id="warehouse-${item.id}" ><td><a class="link-action-removed" href="./warehouseManager/loadWarehouseDetail/${item.id}/"> ${item.id}</a></td>
...
</c:if><c:if test="${param.referName ne 'latitude'}">	<td contenteditable='true' class='edit-value'  propertyToChange='latitude' storedCellValue='${item.latitude}' prefix='${ownerBeanName}Manager/updateWarehouse/${result.id}/${item.id}/'>${item.latitude}</td>
