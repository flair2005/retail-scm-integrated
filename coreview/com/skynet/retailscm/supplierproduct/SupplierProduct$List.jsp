...
<c:set var="supplierProductList" value="${result.supplierProductList}"/>
...
<c:if test="${ empty supplierProductList}" >
 ...
 <c:if test="${not empty result.supplierProductList}" >
    
    
<%

 	SmartList list=(SmartList)pageContext.getAttribute("supplierProductList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/supplierProductList;supplierProductListCurrentPage=${page.pageNumber}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${result.supplierProductList}">
				<tr currentVersion='${item.version}' id="supplierProduct-${item.id}" ><td><a class="link-action-removed" href="./supplierProductManager/loadSupplierProductDetail/${item.id}/"> ${item.id}</a></td>
...
</c:if><c:if test="${param.referName ne 'supplier'}">	<td ><a href='./goodsSupplierManager/loadGoodsSupplierDetail/${item.supplier.id}/'>${item.supplier.id}</a></td>
</c:if>
				<td>
