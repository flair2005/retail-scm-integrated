...
<c:set var="employeeLeaveList" value="${result.employeeLeaveList}"/>
...
<c:if test="${ empty employeeLeaveList}" >
 ...
 <c:if test="${not empty result.employeeLeaveList}" >
    
    
<%

 	SmartList list=(SmartList)pageContext.getAttribute("employeeLeaveList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/employeeLeaveList;employeeLeaveListCurrentPage=${page.pageNumber}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${result.employeeLeaveList}">
				<tr currentVersion='${item.version}' id="employeeLeave-${item.id}" ><td><a class="link-action-removed" href="./employeeLeaveManager/loadEmployeeLeaveDetail/${item.id}/"> ${item.id}</a></td>
<c:if test="${param.referName ne 'who'}">	<td ><a href='./employeeManager/loadEmployeeDetail/${item.who.id}/'>${item.who.id}</a></td>
</c:if><c:if test="${param.referName ne 'type'}">	<td ><a href='./leaveTypeManager/loadLeaveTypeDetail/${item.type.id}/'>${item.type.id}</a></td>
</c:if><c:if test="${param.referName ne 'leaveDurationHour'}">	<td contenteditable='true' class='edit-value'  propertyToChange='leaveDurationHour' storedCellValue='${item.leaveDurationHour}' prefix='${ownerBeanName}Manager/updateEmployeeLeave/${result.id}/${item.id}/'>${item.leaveDurationHour}</td>
