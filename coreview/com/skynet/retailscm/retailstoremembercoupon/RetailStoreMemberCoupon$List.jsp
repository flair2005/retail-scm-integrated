...
<c:set var="retailStoreMemberCouponList" value="${result.retailStoreMemberCouponList}"/>
...
<c:if test="${ empty retailStoreMemberCouponList}" >
 ...
 <c:if test="${not empty result.retailStoreMemberCouponList}" >
    
    
<%

 	SmartList list=(SmartList)pageContext.getAttribute("retailStoreMemberCouponList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/retailStoreMemberCouponList;retailStoreMemberCouponListCurrentPage=${page.pageNumber}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${result.retailStoreMemberCouponList}">
				<tr currentVersion='${item.version}' id="retailStoreMemberCoupon-${item.id}" ><td><a class="link-action-removed" href="./retailStoreMemberCouponManager/loadRetailStoreMemberCouponDetail/${item.id}/"> ${item.id}</a></td>
...
</c:if><c:if test="${param.referName ne 'number'}">	<td contenteditable='true' class='edit-value'  propertyToChange='number' storedCellValue='${item.number}' prefix='${ownerBeanName}Manager/updateRetailStoreMemberCoupon/${result.id}/${item.id}/'>${item.number}</td>
</c:if>
				<td>
