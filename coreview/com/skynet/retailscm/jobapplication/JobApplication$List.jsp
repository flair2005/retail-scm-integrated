...
<c:set var="jobApplicationList" value="${result.jobApplicationList}"/>
...
<c:if test="${ empty jobApplicationList}" >
 ...
 <c:if test="${not empty result.jobApplicationList}" >
    
    
<%

 	SmartList list=(SmartList)pageContext.getAttribute("jobApplicationList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/jobApplicationList;jobApplicationListCurrentPage=${page.pageNumber}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${result.jobApplicationList}">
				<tr currentVersion='${item.version}' id="jobApplication-${item.id}" ><td><a class="link-action-removed" href="./jobApplicationManager/loadJobApplicationDetail/${item.id}/"> ${item.id}</a></td>
