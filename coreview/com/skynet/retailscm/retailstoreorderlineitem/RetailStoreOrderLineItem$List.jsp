...
<c:set var="retailStoreOrderLineItemList" value="${result.retailStoreOrderLineItemList}"/>
...
<c:if test="${ empty retailStoreOrderLineItemList}" >
 ...
 <c:if test="${not empty result.retailStoreOrderLineItemList}" >
    
    
<%

 	SmartList list=(SmartList)pageContext.getAttribute("retailStoreOrderLineItemList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/retailStoreOrderLineItemList;retailStoreOrderLineItemListCurrentPage=${page.pageNumber}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${result.retailStoreOrderLineItemList}">
				<tr currentVersion='${item.version}' id="retailStoreOrderLineItem-${item.id}" ><td><a class="link-action-removed" href="./retailStoreOrderLineItemManager/loadRetailStoreOrderLineItemDetail/${item.id}/"> ${item.id}</a></td>
<c:if test="${param.referName ne 'bizOrder'}">	<td ><a href='./retailStoreOrderManager/loadRetailStoreOrderDetail/${item.bizOrder.id}/'>${item.bizOrder.id}</a></td>
</c:if><c:if test="${param.referName ne 'skuId'}">	<td contenteditable='true' class='edit-value'  propertyToChange='skuId' storedCellValue='${item.skuId}' prefix='${ownerBeanName}Manager/updateRetailStoreOrderLineItem/${result.id}/${item.id}/'>${item.skuId}</td>
...
</c:if><c:if test="${param.referName ne 'amount'}">	<td contenteditable='true' class='edit-value'  propertyToChange='amount' storedCellValue='${item.amount}' prefix='${ownerBeanName}Manager/updateRetailStoreOrderLineItem/${result.id}/${item.id}/'><fmt:formatNumber type="currency"  value="${item.amount}" /></td>
