...
<c:set var="goodsShelfStockCountList" value="${result.goodsShelfStockCountList}"/>
...
<c:if test="${ empty goodsShelfStockCountList}" >
 ...
 <c:if test="${not empty result.goodsShelfStockCountList}" >
    
    
<%

 	SmartList list=(SmartList)pageContext.getAttribute("goodsShelfStockCountList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/goodsShelfStockCountList;goodsShelfStockCountListCurrentPage=${page.pageNumber}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${result.goodsShelfStockCountList}">
				<tr currentVersion='${item.version}' id="goodsShelfStockCount-${item.id}" ><td><a class="link-action-removed" href="./goodsShelfStockCountManager/loadGoodsShelfStockCountDetail/${item.id}/"> ${item.id}</a></td>
...
</c:if><c:if test="${param.referName ne 'shelf'}">	<td ><a href='./goodsShelfManager/loadGoodsShelfDetail/${item.shelf.id}/'>${item.shelf.id}</a></td>
</c:if>
				<td>
