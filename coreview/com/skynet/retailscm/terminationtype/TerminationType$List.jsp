...
<c:set var="terminationTypeList" value="${result.terminationTypeList}"/>
...
<c:if test="${ empty terminationTypeList}" >
 ...
 <c:if test="${not empty result.terminationTypeList}" >
    
    
<%

 	SmartList list=(SmartList)pageContext.getAttribute("terminationTypeList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/terminationTypeList;terminationTypeListCurrentPage=${page.pageNumber}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${result.terminationTypeList}">
				<tr currentVersion='${item.version}' id="terminationType-${item.id}" ><td><a class="link-action-removed" href="./terminationTypeManager/loadTerminationTypeDetail/${item.id}/"> ${item.id}</a></td>
...
</c:if><c:if test="${param.referName ne 'baseDescription'}">	<td contenteditable='true' class='edit-value'  propertyToChange='baseDescription' storedCellValue='${item.baseDescription}' prefix='${ownerBeanName}Manager/updateTerminationType/${result.id}/${item.id}/'>${item.baseDescription}</td>
