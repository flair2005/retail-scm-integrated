...
<c:set var="levelThreeDepartmentList" value="${result.levelThreeDepartmentList}"/>
...
<c:if test="${ empty levelThreeDepartmentList}" >
 ...
 <c:if test="${not empty result.levelThreeDepartmentList}" >
    
    
<%

 	SmartList list=(SmartList)pageContext.getAttribute("levelThreeDepartmentList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/levelThreeDepartmentList;levelThreeDepartmentListCurrentPage=${page.pageNumber}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${result.levelThreeDepartmentList}">
				<tr currentVersion='${item.version}' id="levelThreeDepartment-${item.id}" ><td><a class="link-action-removed" href="./levelThreeDepartmentManager/loadLevelThreeDepartmentDetail/${item.id}/"> ${item.id}</a></td>
<c:if test="${param.referName ne 'belongsTo'}">	<td ><a href='./levelTwoDepartmentManager/loadLevelTwoDepartmentDetail/${item.belongsTo.id}/'>${item.belongsTo.id}</a></td>
</c:if><c:if test="${param.referName ne 'name'}">	<td contenteditable='true' class='edit-value'  propertyToChange='name' storedCellValue='${item.name}' prefix='${ownerBeanName}Manager/updateLevelThreeDepartment/${result.id}/${item.id}/'>${item.name}</td>
