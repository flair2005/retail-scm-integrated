...
<c:set var="warehouseAssetList" value="${result.warehouseAssetList}"/>
...
<c:if test="${ empty warehouseAssetList}" >
 ...
 <c:if test="${not empty result.warehouseAssetList}" >
    
    
<%

 	SmartList list=(SmartList)pageContext.getAttribute("warehouseAssetList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/warehouseAssetList;warehouseAssetListCurrentPage=${page.pageNumber}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${result.warehouseAssetList}">
				<tr currentVersion='${item.version}' id="warehouseAsset-${item.id}" ><td><a class="link-action-removed" href="./warehouseAssetManager/loadWarehouseAssetDetail/${item.id}/"> ${item.id}</a></td>
...
</c:if><c:if test="${param.referName ne 'owner'}">	<td ><a href='./warehouseManager/loadWarehouseDetail/${item.owner.id}/'>${item.owner.id}</a></td>
</c:if>
				<td>
