...
<c:set var="retailStoreOrderPaymentGroupList" value="${result.retailStoreOrderPaymentGroupList}"/>
...
<c:if test="${ empty retailStoreOrderPaymentGroupList}" >
 ...
 <c:if test="${not empty result.retailStoreOrderPaymentGroupList}" >
    
    
<%

 	SmartList list=(SmartList)pageContext.getAttribute("retailStoreOrderPaymentGroupList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/retailStoreOrderPaymentGroupList;retailStoreOrderPaymentGroupListCurrentPage=${page.pageNumber}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${result.retailStoreOrderPaymentGroupList}">
				<tr currentVersion='${item.version}' id="retailStoreOrderPaymentGroup-${item.id}" ><td><a class="link-action-removed" href="./retailStoreOrderPaymentGroupManager/loadRetailStoreOrderPaymentGroupDetail/${item.id}/"> ${item.id}</a></td>
...
</c:if><c:if test="${param.referName ne 'cardNumber'}">	<td contenteditable='true' class='edit-value'  propertyToChange='cardNumber' storedCellValue='${item.cardNumber}' prefix='${ownerBeanName}Manager/updateRetailStoreOrderPaymentGroup/${result.id}/${item.id}/'>${item.cardNumber}</td>
</c:if>
				<td>
