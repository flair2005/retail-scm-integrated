...
<c:set var="employeeQualifierList" value="${result.employeeQualifierList}"/>
...
<c:if test="${ empty employeeQualifierList}" >
 ...
 <c:if test="${not empty result.employeeQualifierList}" >
    
    
<%

 	SmartList list=(SmartList)pageContext.getAttribute("employeeQualifierList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/employeeQualifierList;employeeQualifierListCurrentPage=${page.pageNumber}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${result.employeeQualifierList}">
				<tr currentVersion='${item.version}' id="employeeQualifier-${item.id}" ><td><a class="link-action-removed" href="./employeeQualifierManager/loadEmployeeQualifierDetail/${item.id}/"> ${item.id}</a></td>
...
</c:if><c:if test="${param.referName ne 'qualifiedTime'}">	<td contenteditable='true' class='edit-value'  propertyToChange='qualifiedTime' storedCellValue='${item.qualifiedTime}' prefix='${ownerBeanName}Manager/updateEmployeeQualifier/${result.id}/${item.id}/'><fmt:formatDate pattern="yyyy-MM-dd" value="${item.qualifiedTime}" /></td>
