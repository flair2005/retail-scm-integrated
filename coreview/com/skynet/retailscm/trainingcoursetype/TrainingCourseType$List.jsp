...
<c:set var="trainingCourseTypeList" value="${result.trainingCourseTypeList}"/>
...
<c:if test="${ empty trainingCourseTypeList}" >
 ...
 <c:if test="${not empty result.trainingCourseTypeList}" >
    
    
<%

 	SmartList list=(SmartList)pageContext.getAttribute("trainingCourseTypeList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/trainingCourseTypeList;trainingCourseTypeListCurrentPage=${page.pageNumber}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${result.trainingCourseTypeList}">
				<tr currentVersion='${item.version}' id="trainingCourseType-${item.id}" ><td><a class="link-action-removed" href="./trainingCourseTypeManager/loadTrainingCourseTypeDetail/${item.id}/"> ${item.id}</a></td>
...
</c:if><c:if test="${param.referName ne 'name'}">	<td contenteditable='true' class='edit-value'  propertyToChange='name' storedCellValue='${item.name}' prefix='${ownerBeanName}Manager/updateTrainingCourseType/${result.id}/${item.id}/'>${item.name}</td>
