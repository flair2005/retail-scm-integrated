...
<c:set var="accountingDocumentLineList" value="${result.accountingDocumentLineList}"/>
...
<c:if test="${ empty accountingDocumentLineList}" >
 ...
 <c:if test="${not empty result.accountingDocumentLineList}" >
    
    
<%

 	SmartList list=(SmartList)pageContext.getAttribute("accountingDocumentLineList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/accountingDocumentLineList;accountingDocumentLineListCurrentPage=${page.pageNumber}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${result.accountingDocumentLineList}">
				<tr currentVersion='${item.version}' id="accountingDocumentLine-${item.id}" ><td><a class="link-action-removed" href="./accountingDocumentLineManager/loadAccountingDocumentLineDetail/${item.id}/"> ${item.id}</a></td>
...
</c:if><c:if test="${param.referName ne 'amount'}">	<td contenteditable='true' class='edit-value'  propertyToChange='amount' storedCellValue='${item.amount}' prefix='${ownerBeanName}Manager/updateAccountingDocumentLine/${result.id}/${item.id}/'><fmt:formatNumber type="currency"  value="${item.amount}" /></td>
...
</c:if><c:if test="${param.referName ne 'accountingSubject'}">	<td ><a href='./accountingSubjectManager/loadAccountingSubjectDetail/${item.accountingSubject.id}/'>${item.accountingSubject.id}</a></td>
</c:if>
				<td>
