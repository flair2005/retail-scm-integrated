...
<c:set var="viewList" value="${result.viewList}"/>
...
<c:if test="${ empty viewList}" >
 ...
 <c:if test="${not empty result.viewList}" >
    
    
<%

 	SmartList list=(SmartList)pageContext.getAttribute("viewList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/viewList;viewListCurrentPage=${page.pageNumber}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${result.viewList}">
				<tr currentVersion='${item.version}' id="view-${item.id}" ><td><a class="link-action-removed" href="./viewManager/loadViewDetail/${item.id}/"> ${item.id}</a></td>
