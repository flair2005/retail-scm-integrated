...
<c:set var="consumerOrderLineItemList" value="${result.consumerOrderLineItemList}"/>
...
<c:if test="${ empty consumerOrderLineItemList}" >
 ...
 <c:if test="${not empty result.consumerOrderLineItemList}" >
    
    
<%

 	SmartList list=(SmartList)pageContext.getAttribute("consumerOrderLineItemList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/consumerOrderLineItemList;consumerOrderLineItemListCurrentPage=${page.pageNumber}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${result.consumerOrderLineItemList}">
				<tr currentVersion='${item.version}' id="consumerOrderLineItem-${item.id}" ><td><a class="link-action-removed" href="./consumerOrderLineItemManager/loadConsumerOrderLineItemDetail/${item.id}/"> ${item.id}</a></td>
<c:if test="${param.referName ne 'bizOrder'}">	<td ><a href='./consumerOrderManager/loadConsumerOrderDetail/${item.bizOrder.id}/'>${item.bizOrder.id}</a></td>
</c:if><c:if test="${param.referName ne 'skuId'}">	<td contenteditable='true' class='edit-value'  propertyToChange='skuId' storedCellValue='${item.skuId}' prefix='${ownerBeanName}Manager/updateConsumerOrderLineItem/${result.id}/${item.id}/'>${item.skuId}</td>
...
</c:if><c:if test="${param.referName ne 'price'}">	<td contenteditable='true' class='edit-value'  propertyToChange='price' storedCellValue='${item.price}' prefix='${ownerBeanName}Manager/updateConsumerOrderLineItem/${result.id}/${item.id}/'><fmt:formatNumber type="currency"  value="${item.price}" /></td>
...
</c:if><c:if test="${param.referName ne 'amount'}">	<td contenteditable='true' class='edit-value'  propertyToChange='amount' storedCellValue='${item.amount}' prefix='${ownerBeanName}Manager/updateConsumerOrderLineItem/${result.id}/${item.id}/'><fmt:formatNumber type="currency"  value="${item.amount}" /></td>
</c:if>
				<td>
