...
<c:set var="employeePerformanceList" value="${result.employeePerformanceList}"/>
...
<c:if test="${ empty employeePerformanceList}" >
 ...
 <c:if test="${not empty result.employeePerformanceList}" >
    
    
<%

 	SmartList list=(SmartList)pageContext.getAttribute("employeePerformanceList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/employeePerformanceList;employeePerformanceListCurrentPage=${page.pageNumber}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${result.employeePerformanceList}">
				<tr currentVersion='${item.version}' id="employeePerformance-${item.id}" ><td><a class="link-action-removed" href="./employeePerformanceManager/loadEmployeePerformanceDetail/${item.id}/"> ${item.id}</a></td>
...
</c:if><c:if test="${param.referName ne 'performanceComment'}">	<td contenteditable='true' class='edit-value'  propertyToChange='performanceComment' storedCellValue='${item.performanceComment}' prefix='${ownerBeanName}Manager/updateEmployeePerformance/${result.id}/${item.id}/'>${item.performanceComment}</td>
</c:if>
				<td>
