...
<c:set var="objectAccessList" value="${result.objectAccessList}"/>
...
<c:if test="${ empty objectAccessList}" >
 ...
 <c:if test="${not empty result.objectAccessList}" >
    
    
<%

 	SmartList list=(SmartList)pageContext.getAttribute("objectAccessList"); 
...
%>
    ...
    <div class="row" style="font-size: 30px;">
		<div class="col-xs-12 col-md-12" style="padding-left:20px">
		<i class='fa fa-list-ol'></i> ${userContext.localeMap['object_access']}(${totalCount})
		<a href="./${ownerBeanName}Manager/addObjectAccess/${result.id}/"><i class="fa fa-plus-square" aria-hidden="true"></i></a>
		 </div>
 </div>
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/objectAccessList;objectAccessListCurrentPage=${page.pageNumber}/' 
...
<c:if test="${param.referName ne 'displayName'}">
	<th>${userContext.localeMap['object_access.display_name']}</th>
</c:if>
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${result.objectAccessList}">
				<tr currentVersion='${item.version}' id="objectAccess-${item.id}" ><td><a class="link-action-removed" href="./objectAccessManager/loadObjectAccessDetail/${item.id}/"> ${item.id}</a></td>
<c:if test="${param.referName ne 'displayName'}">	<td contenteditable='true' class='edit-value'  propertyToChange='displayName' storedCellValue='${item.displayName}' prefix='${ownerBeanName}Manager/updateObjectAccess/${result.id}/${item.id}/'>${item.displayName}</td>
...
</c:if><c:if test="${param.referName ne 'app'}">	<td ><a href='./userAppManager/loadUserAppDetail/${item.app.id}/'>${item.app.id}</a></td>
</c:if>
				<td>
