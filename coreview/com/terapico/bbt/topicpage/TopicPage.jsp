...
<c:set var="ownerBeanName" value="topicPage" scope="request"/>
<c:set var="ownerClassName" value="TopicPage" scope="request"/>
...
<div class="row">
	...
	<c:if test='${(not empty userContext.accessTokens["topicFilterList"]) or ignoreListAccessControl}'>
			<li><a data-toggle="tab" href="#topicFilterList" class="disabled"> ${userContext.localeMap['topic_filter']}</a></li>
	</c:if>
	<c:if test='${(not empty userContext.accessTokens["topicList"]) or ignoreListAccessControl}'>
			<li><a data-toggle="tab" href="#topicList" class="disabled"> ${userContext.localeMap['topic']}</a></li>
	</c:if>
...
<div class="row" desc="show parent objects">
	...
	<c:set var="topicPage" value="${ result}" scope="request" />
<jsp:include page="/coreview/com/terapico/bbt/topicpage/TopicPage$Summary.jsp" />
...
<jsp:include page="/coreview/com/terapico/bbt/community/Community$Info.jsp" />
		...
		<c:if test='${not empty userContext.accessTokens["topicFilterList"] or ignoreListAccessControl}'>
		<div id="topicFilterList" class="tab-pane fade sublist" refer-name="topic_page">
			<jsp:include page="/coreview/com/terapico/bbt/topicfilter/TopicFilter$List.jsp" >
				<jsp:param name="referName" value="topicPage"/>
			</jsp:include>
		</div>
	</c:if>
	<c:if test='${not empty userContext.accessTokens["topicList"] or ignoreListAccessControl}'>
		<div id="topicList" class="tab-pane fade sublist" refer-name="topic_page">
			<jsp:include page="/coreview/com/terapico/bbt/topic/Topic$List.jsp" >
				<jsp:param name="referName" value="topicPage"/>
			</jsp:include>
		</div>
	</c:if>
