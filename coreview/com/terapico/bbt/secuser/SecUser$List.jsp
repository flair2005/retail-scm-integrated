...
</c:if><c:if test="${param.referName ne 'lastLogin'}">	<td contenteditable='true' class='edit-value'  propertyToChange='lastLogin' storedCellValue='${item.lastLogin}' prefix='${ownerBeanName}Manager/updateSecUser/${result.id}/${item.id}/'><fmt:formatDate pattern="yyyy-MM-dd" value="${item.lastLogin}" /></td>
