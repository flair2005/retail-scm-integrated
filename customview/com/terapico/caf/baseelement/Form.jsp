...
<div class="panel panel-default">
  ...
  <div class="panel-body">
    ...
    <form class="mainform1"  method="post" 
    	action="./<c:forEach var="action" items="${result.actions}">${action.name}</c:forEach>/<c:forEach var="field" items="${result.fields}">${field.name}/</c:forEach>">
		<c:forEach var="field" items="${result.fields}" varStatus="status">
		<div class="col-sm-12">
			<input id="fl${status.index}" class="form-control input-sm" 
			placeHolder="${field.name}" value="${field.defaultValue}" name="${field.name}"
